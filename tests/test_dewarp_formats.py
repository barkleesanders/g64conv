...
from g64conv.dewarp import DewarpSpec, dewarp
...
def test_double_view_wall_order_and_no_mirror(tmp_path, fisheye_video, ffmpeg):
    outs = dewarp(fisheye_video, str(tmp_path), DewarpSpec(mode="double", width=720))
...
def test_panorama_mode(tmp_path, fisheye_video, ffmpeg):
    outs = dewarp(fisheye_video, str(tmp_path), DewarpSpec(mode="panorama", width=1440))
    assert len(outs) == 1
